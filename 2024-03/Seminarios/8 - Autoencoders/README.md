...
2. Build the Docker image by running the following command in the directory containing your `Dockerfile`:
   ...
   ```bash
   docker build -t cpe727-autoencoder .
   ```
...
```bash
docker stop cpe727-autoencoder-cnt
docker rm cpe727-autoencoder-cnt
```
...
2. Build and tag your image, then push it to Docker Hub:
   ```bash
   docker build -t <username>/cpe727-autoencoder .
   docker push <username>/cpe727-autoencoder
   ```
...
```bash
singularity pull --disable-cache cpe727-autoencoder.sif docker://<username>/cpe727-autoencoder
```
